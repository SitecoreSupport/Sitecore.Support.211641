...
var SitecoreMediaUploader = Class.create({
    ...
    onUploadError: function (event) {
        /* Sitecore.Support.211641 fix start */
        /* Sitecore.Support.211641 fix, previously the "event.id" was used which is undefined. 
         * Changed to the event.details[0].target.id  as it should be used as in all other methods. */

        console.warn("upload error. file: %s, message: %s", event.details[0].target.id, event.status);
        this.hadErrors = true;

        var row = $(event.details[0].target.id);
        /* Sitecore.Support.211641 fix end */
